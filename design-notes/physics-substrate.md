...
The failure outcome for an intentional false attack is not the compromised state the template's FailureSpec would normally route to (two-knee-drop Seoi-nage would otherwise land in `TORI_ON_BOTH_KNEES_UKE_STANDING` with 5 tick recovery). Instead, HAJ-49 adds a new `TACTICAL_DROP_RESET` compromised state (Part 6.3): shallow CoM dip, both feet near-weighted, 1-tick recovery (HAJ-50), no counter-window vulnerability, ~0.005 composure cost. This is the mechanical representation of a planned cost: the entry was shallow, tori is already recovering, and uke has nothing to exploit. HAJ-50 also adds a physics-driven path into this state: a drop-variant throw that fails with kuzushi + force signature below `TACTICAL_DROP_SIGNATURE_FLOOR` at commit routes to `TACTICAL_DROP_RESET` regardless of the motivation label, because the two cases (deliberate fake, unlabelled low-signature drop) are mechanically indistinguishable.
...
**TACTICAL_DROP_RESET (HAJ-49 / HAJ-50).**
...
- Prose register: compact two-beat line (`[throw] Gaba drops on Tai-otoshi. Nothing there. Back up.`) rather than the drawn-out `(tag; recovery N tick(s))` narration other failures get. The log should slide past the event the way uke's reading slides past tori's fake.
