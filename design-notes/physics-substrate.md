...
- **Collar grip** — HAJ-161 splits the prior bare collar entry into two positional sub-types because the head-as-output computation needs a mechanically honest referent: a *lapel* grip steers the torso, not the head. Both sub-types share the collar family for plan-layer purposes (PULL_LAPEL / DEEPEN_LAPEL still target either) but only the collar family drives uke's HEAD body-part state.
  ...
  - **COLLAR_BACK (oku-eri, nape grip)** — the hand reaches over uke's shoulder and grips the collar behind the neck. Maximum trunk-rotation moment arm and the only grip in the vocabulary that drives uke's head as a coupled output. Setup grip for sumi-gaeshi (over-the-top), kata-guruma, and the high-collar entry to forward-rotation throws (uchi-mata, harai-goshi, o-guruma) where the head-forward kuzushi rotates the body around the sweep / fulcrum. Also a documented setup for backward-rotation reaps: per FloGrappling's "Ultimate Guide to Gripping" (Tom Grant, 2016), Teddy Riner uses the high-collar grip to set up "his feared uchi mata and osoto gari throws," with the over-the-shoulder grip driving uke onto the reaped leg's heel.
...
**3–6. Non-scoring motivations.** Four distinct mechanical paths that fire the same low-signature drop-variant commit pathway for different tactical reasons. Each is gated by its own conjunction of preconditions plus a per-tick probability roll, so they're mutually exclusive per tick (first match wins). All four select from the same drop-variant priority order (TAI_OTOSHI → KO_UCHI_GARI → drop-SEOI → O_UCHI_GARI) and all four route to `TACTICAL_DROP_RESET` (Part 6.3) on failure. They carry a `commit_motivation: CommitMotivation` label on the Action and THROW_ENTRY event so prose and analytics can tell them apart — but **the referee does not read the motivation**. The referee's false-attack shido evaluation operates on the commit itself: what tori did on the mat, not why.

- **CLOCK_RESET (HAJ-49).** Kumi-kata clock in the pre-shido zone [18, 29), composure and fight_iq above the desperation bar, `style_dna["false_attack_tendency"]` ≥ 0.40. Tori resets their own passivity clock. Modern European competition staple (French INSEP lineage).
- **GRIP_ESCAPE (HAJ-67).** Opponent is dominant in the grip war (grip-delta ≥ 0.40), tori's own grip integrity is compromised (no edge deeper than POCKET), and composure has slipped below ~55%. The fake is cover to rip off the dyad and reset grips — a pragmatic option when the grip war has been lost but panic hasn't set in.
- **SHIDO_FARMING (HAJ-67).** Opponent has been passive (their kumi-kata clock ≥ 15), tori has no real scoring opportunity (no perceived signature ≥ 0.40), and `style_dna["shido_farming_tendency"]` ≥ 0.45. Tori poses an attack to grind the referee into issuing a shido against uke rather than tori.
- **STAMINA_DESPERATION (HAJ-67).** Cardio below 30%, at least one shido already taken, average hand fatigue above 60% (proxy for "cannot generate kuzushi"). A cooked, penalized fighter falls into anything to buy a breather on the mat — mechanically forced rather than tactically chosen.

Per-tick firing is probabilistic across all four so no single motivation monopolizes the attack space. Probabilities are calibration targets; current values: CLOCK_RESET tendency × 0.10, STAMINA_DESPERATION 0.15, GRIP_ESCAPE 0.10, SHIDO_FARMING 0.06.

The failure outcome for any non-scoring-motivation commit is `TACTICAL_DROP_RESET` (Part 6.3): shallow CoM dip, both feet near-weighted, 1-tick recovery, no counter-window vulnerability, ~0.005 composure cost. HAJ-50 also adds a physics-driven path into this state: a drop-variant throw that fails with kuzushi + force signature below `TACTICAL_DROP_SIGNATURE_FLOOR` at commit routes to `TACTICAL_DROP_RESET` regardless of the motivation label, because the two cases (deliberate fake, unlabelled low-signature drop) are mechanically indistinguishable.

The commit log line surfaces the motivation so all six are distinguishable at a glance: a normal commit has no tag, offensive desperation tags as `(offensive desperation; gate bypassed: …)`, and a non-scoring motivation tags as `(commit motivation: clock_reset | grip_escape | shido_farming | stamina_desperation)`. On failure, each non-scoring motivation emits its own compact two-beat prose line (see Part 6.3 for templates).
...
**TACTICAL_DROP_RESET (HAJ-49 / HAJ-50 / HAJ-67).**
- Configuration: shallow CoM dip (≈ 0.05 m), both feet near-weighted (0.55 / 0.45), upper-body posture unchanged. Not structurally compromised — tori is already returning to stance as the state resolves.
- Origin: two paths into this state. (1) The motivation-label path — any of the four non-scoring commit motivations (Part 3.3.1) is unconditionally routed here because tori explicitly chose the fake. (2) The physics-driven path — a drop-variant throw (Tai-otoshi, Ko-uchi-gari, drop Seoi, O-uchi-gari) that failed with kuzushi + force signature below `TACTICAL_DROP_SIGNATURE_FLOOR` (≈ 0.20) at commit. This second path catches desperation commits and noise-triggered commits that happen to fire on a drop variant without any real loading of uke's CoM: physically the two are indistinguishable from a deliberate fake, and the failure outcome reflects that.
- Counter vulnerabilities: none (HAJ-50). `counter_bonuses` is empty. Uke cannot score an osaekomi transition against a tori who is already standing back up; there is no loaded body to scoop under for Ura-nage; there is no hikite to pull down into Tate-shiho-gatame. The only real cost of entering this state is a single tick of no-offense — plus whatever the normal grip-graph maintenance rolls cost anyway.
- Recovery: 1 tick (HAJ-50 minimum). The lightest compromised state in the table. Composure drop is ≈ 0.005 — effectively zero, because this was a tactical cost rather than a failed attempt.
- Prose register (HAJ-67): compact two-beat line per motivation — each non-scoring motivation renders a distinct template so a reader can tell *why* tori faked at a glance:
    - `CLOCK_RESET` → `[throw] Gaba drops on Tai-otoshi. Nothing there. Back up.`
    - `GRIP_ESCAPE` → `[throw] Gaba breaks contact with a half-Tai-otoshi. Grips reset.`
    - `SHIDO_FARMING` → `[throw] Gaba poses an attack — Tai-otoshi. Kept moving; clock ticks.`
    - `STAMINA_DESPERATION` → `[throw] Gaba falls into Tai-otoshi. Out of gas, out of options.`
  A commit routed here by the physics-driven discriminator (no motivation label) falls back to the `CLOCK_RESET` template. The log should slide past the event the way uke's reading slides past tori's fake.
