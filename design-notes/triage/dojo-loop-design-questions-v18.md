...
**Dojo Records System (Q16):**
- **Q16a** — What dimensions of the dojo's history are surfaced — lineage, philosophy, signature moves, competition results, current and former roster?
- **Q16b** — Is the dojo records view always available, or does it unlock with a facility milestone (the way the roster's notepad-to-computer transition works in Q11)?
- **Q16c** — How does the records view interact with the pinning mechanic from Q11b? Are pinned characters surfaced in the dojo records too?
- **Q16d** — What is the shape of "philosophies and tenets" as surfaced data? Generated from cultural-input history? Authored phrases? Player-named?
- **Q16e** — Does the dojo records view become a shareable artifact (screenshot-friendly, exportable, eventually Steam Workshop-style)?
- **Q16f** — How does the records system interact with the lineage data model from Q15 — does the records view of dojo X show all generations of senseis who ran it, including pre-player history?
...
### Q16 — Dojo Records System (story legibility infrastructure). OPEN.

**The question:** how does the dojo's accumulated history — its origins, its philosophies, its champion records, its current and former roster — surface to the player as a legible artifact they can navigate, reflect on, and find their own stories inside?

**Why this is its own question, not a sub-question of Q11 or Q15:**

- **Q11** answered how the *roster* surfaces students (notepad → computer evolution, granular toggle, alumni view, pinning via favorites tab). The roster is *student-centric* — it organizes information about individual judoka.
- **Q15** answered the *lineage data model* — every sensei, dojo, and student carries lineage-aware fields from 1.0 onward. This is the data substrate.
- **Q16 is the dojo-as-institution-centric view.** It asks: how does the dojo *itself* surface its own story? Not as a list of students (Q11) or as a data model (Q15), but as a legible institutional history — *this dojo, where it came from, what it stands for, what it has produced.*

This question emerged from a v0.6 strategy session insight: *Hajime's word-of-mouth network effect lives in shareable stories, and the simulation needs a rendering layer to make its outputs legible.* The cultural feedback loop produces emergence over decades; without a system that surfaces that emergence as story, the simulation's outputs stay invisible to the player and unshareable in the world.

**The four dimensions sketched in the strategy session:**

1. **Lineage origin.** What sensei the dojo evolved from. Where it began. The sensei's father's dojo. The basement origin. The chain of senseis if the player has played multiple successor-start runs (per Q15).
2. **Philosophies, tenets, and signature moves.** What this dojo *stands for* and *teaches.* Generated from accumulated cultural-input history (the six cultural inputs — see master doc and v17 Q8). Surfaced as legible phrases or motifs the player can read, not raw axis values.
3. **Records of accomplishment.** State champions, local champions, national qualifiers, Olympians produced. Tournament wins, podium counts, ranking peaks of alumni.
4. **Current and former roster.** Ranks, ages, weights, status of current students. Former students by current location and status (per Q11a — the alumni view).

**Strategic implication, lifted from strategy doc v0.6 Commitment #21:** *the simulation produces emergence; rendering systems (dojo records, pinning) make that emergence legible. Both halves are required — emergence without legibility is invisible; legibility without emergence is hollow.* The Q16 records system is half of the legibility infrastructure (the other half being the pinning mechanic already answered in Q11b).

**Architectural observations (preliminary):**

- The records view is read-only from the simulation's perspective — it surfaces accumulated state, doesn't author it.
- The records view is *probably* a separate UI surface from the roster, even though both touch student data. The roster is operational (this is who's training right now, who needs attention); the records are reflective (this is what we are, where we came from, what we've done).
- The records view interacts deeply with Q15's lineage data model. The records of a successor-start dojo show not just the current sensei's record but inherited records from the prior generation. *This is the point of the lineage data model from 1.0 — Q15 was always partly setup for Q16.*
- The records view may be one of the natural "shareable artifacts" of the game. A player might screenshot their dojo's records page after fifteen in-game years to share on Reddit or Discord. This pulls forward S2 (the traveling sentence) into UI — *the dojo's record page should look good, because it will be one of the most-screenshotted things in the game.*

**Why this matters strategically (per strategy doc v0.6):**

The deep-sim genre's word-of-mouth network effect is largely built on shareable stories. RimWorld grew on Reddit through colonist tragedies. CK3 grew on r/CrusaderKings through dynastic insanity. DF grew through legends-mode stories. The shareable artifact in each case is *the player's own simulation output rendered as legible narrative.* Hajime's equivalent is the dojo's records page after years of play. If the records page is well-designed, it becomes the artifact the audience uses to tell each other Hajime stories. If it doesn't exist or is poorly designed, the simulation's stories stay locked inside individual players' heads.

**Implications for other questions:**

- **Q11 (roster):** the records view is the institutional sibling of the roster. They share data but serve different purposes. The roster is current-state operational; the records are accumulated-state reflective. Both probably become accessible at the same notepad-to-computer transition, or the records may unlock at a separate facility milestone.
- **Q11a (alumni):** the records' "former roster" dimension *is* the alumni view from Q11a, surfaced from the institutional perspective rather than the student-list perspective. Same data, different framing.
- **Q11b (pinning):** the pinning mechanic from Q11b interacts with records — pinned characters probably appear with prominence in the records view, and pinning a character might be one of the natural actions a player takes from the records page.
- **Q12 (rankings):** competition results data flows from the rankings system into the records' "accomplishments" dimension. If Marco won state at green belt, that's a roster fact (Q11) AND a records fact (Q16) — but the records view is where it gets *narrative weight* as part of the dojo's institutional story.
- **Q15 (lineage):** Q16 is the surfacing layer for the data substrate Q15 commits to. Q15 answered "what data do we keep"; Q16 asks "how does it become visible as story."
- **Q14 (Career Mode events):** narrative events should write into the records. The Inheritance Event (Q13) creates a permanent records entry — "year 2: father offered the dojo, declined / accepted." The records are the durable narrative trace of the choices the player made.
- **Q9 (Anchoring Scenes):** the closing Anchoring Scene (the twins as disciples) is a records-page moment as much as a narrative-event moment. The records page surfaces "this dojo produced these two black belts who now teach here" as a permanent legible truth.

**Sub-questions:**

- **Q16a — Surfaced dimensions.** What's the minimum viable set for January? Probably: dojo origin (lineage), current roster summary, accomplishments-to-date list, top 1–2 cultural signature phrases. Deferrable: full philosophy generation, full alumni network with destinations, multi-generation lineage chains.
- **Q16b — Unlock timing.** Is the records view always available from game start, or does it unlock with a facility milestone (parallel to Q11's notepad → computer transition)? Probably *always-available* but *grows in richness* as the dojo accumulates history. A year-one dojo's records page is sparse and clean; a year-twenty dojo's is rich and layered. The visual evolution of the records page IS the dojo's story.
- **Q16c — Pinning interaction.** Pinned characters from Q11b should probably appear with visual prominence on the dojo records page. Possibly a "watching this judoka" indicator that surfaces their current status without leaving the records view. The records view might also be the natural location *to* pin from — surfacing characters worth following.
- **Q16d — Philosophy and tenets generation.** This is the hardest sub-question. What does "philosophy" look like as a surfaced phrase? Options: (a) generated from cultural-input history using procedural phrase templates ("This dojo trains hard. They prefer ne-waza. They send students to compete early."); (b) authored fixed phrases drawn from a library, selected based on cultural-input axis values; (c) player-named, where the player writes their dojo's stated philosophy at character creation or unlocks the ability to name it; (d) some combination of the above. The Cranford voice profile — weight-trapping, fulcrum-mismatch, posture-driven stamina drain — is candidate language for what authentic dojo-philosophy phrasing reads like. *Note: the right answer might be (b) plus (c) — a library of authored phrases that the simulation selects based on cultural state, plus an optional player-naming layer.*
- **Q16e — Records as shareable artifact.** Is the records page screenshot-friendly by design? Are there visual treatments that make it "feel like" a dojo's actual wall of records (frames, photographs, legacy display)? Does it eventually export — to image, to text, to Steam Workshop-style sharing? Out of January scope, but the architectural decision to *make the records page worth screenshotting* affects design from now.
- **Q16f — Multi-generation lineage view.** When a player runs a successor-start campaign (per Q15), does the records page show *all* generations of the dojo's history, or only the player's own tenure? Probably all — that's the Q15 lineage payoff — but this requires careful UX. A 2-generation dojo's records is straightforward; a 5-generation dojo's records needs hierarchy.
- **Q16g — Records updating frequency.** Does the records view update in real-time, weekly with the roundup, or after specific events (belt promotions, tournament wins, character milestones)? Probably event-triggered for the major entries, with periodic recomputation of summary stats. Should never feel like a stale dashboard.
...
*Document version: v18 (Q16 — Dojo Records System added; v17 → v18 transition is purely additive, no existing questions or answers modified).*
